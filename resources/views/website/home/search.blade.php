                    ...
                    <form action="{{ route('website.package.search') }}" method="GET" id="search-form">
                        ...
                        <button class="btn btn-outline-secondary bg-white ms-2 border-0 rounded-pill ms-n3" type="submit">
                            ...
                            <div class="card-body bg-white">
                                <button class="rounded-5 border-2 btn btn-outline-warning text-black px-3 py-1 grow-2" type="submit">Apply</button>
                            ...
                            @foreach($searchResults as $search)
                                    <a href="" class="text-decoration-none">
                                        <div class="card mb-3 p-3 border-0 shadow grow-2">
                                            <div class="row g-0">
                                                <div class="col-lg-5">
                                                    <img src="{{ asset($search->package_image) }}" class="h-100 img-fluid card-img-top rounded-4 shadow grow-1">
                                                </div>
                                                <div class="col-lg-7">
                                                    <div class="card-body">
                                                        <h5 class="card-title">{{ $search->tour_title }}</h5>
                                                        <p class="card-text">{{ Carbon\Carbon::createFromDate($search->tour_start_date)->diffInDays($search->tour_end_date) }} Days</p>
                                                        @for($i = 1; $i <= 5; $i++)
                                                            @if($search->tour_rating >= $i)
                                                                <i class="fa-solid fa-star text-orange"></i>
                                                            @else
                                                                <i class="fa-regular fa-star text-orange"></i>
                                                            @endif
                                                        @endfor
                                                        <p class="card-text"><span><i class="las la-map-marker"></i></span>{{ $search->tour_address }}</p>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-8">
                                                                @foreach ($lowestPrices as $lowestPrice)
                                                                    @if($search->id == $lowestPrice->package_id)
